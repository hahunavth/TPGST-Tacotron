...
#     # vocab =
#     vocab_size=152
#         # 'PE !,.?' + JAMO_LEADS + JAMO_VOWELS + JAMO_TAILS
#
...
class ConfigArgs:
    ...
    """
    data_path = './data/processed_vlsp_data_phoneme_level'  # DEFAULT '../../data/kss'
    mel_dir, mag_dir = 'k_format_mels_tuyen_dv', 'mags'
    meta = "meta_data.csv"  # 'transcript.v.1.4.txt' # DEFAULT 'transcript.v.1.3.txt'
    testset = './data/processed_vlsp_data_phoneme_level/val.txt'
    logdir = 'logs'  # log directory
    sampledir = 'samples'  # directory where samples are located
    mem_mode = False  # load all of the mel spectrograms into memory
    ...
    tp_start = 100000
    sr = 16000  # DEFAULT 22050 # sampling rate
    n_fft = 1024  # n point Fourier transform
    n_mags = n_fft//2 + 1  # magnitude spectrogram frequency
    n_mels = 80  # mel spectrogram dimension
    hop_length = 256  # hop length as a number of frames
    ...
    batch_size = 48  # DEFAULT 32 # for training
    test_batch = 16  # for test
    max_step = 200000  # 400000 # maximum training step
    lr = 0.001  # learning rate
    warm_up_steps = 4000.0  # warm up learning rate
    # lr_decay_step = 50000 # actually not decayed per this step
    # lr_step = [100000, 300000] # multiply 1/10
